<?php
    ...
    abstract class Model {
        ...
        final private function isFieldValueValid(string $fieldName, $fieldValue) {
        ...
        final private function checkFieldList(array $data){
            ...
            foreach ($requestedFieldNames as $requestedFieldName) {
                ...
                if ( !$fields[$requestedFieldName]->isValid($data[$requestedFieldName]) ){
                    throw new \Exception('Field value ' .$requestedFieldName . ' is not valid.');
                }
            }
        }
        ...
        final public function deleteForUserById(int $id) {
            $tableName = $this->getTableName();

            $sql = "UPDATE {$tableName} SET is_deleted=1 WHERE {$tableName}_id = ?";
            $prep = $this->dbc->getConnection()->prepare($sql);
            return $prep->execute($values);
        }
